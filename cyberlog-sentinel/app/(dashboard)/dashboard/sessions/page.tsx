              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '3rem', color: '#8892A4' }}>
                    {jobId
                      ? 'No SSH sessions found in this log file. Sessions are reconstructed from pam_unix session opened/closed pairs.'
                ...
                sessions.map(s => {
                  ...
                  return (
                    <>
                      ...
                      {expandedId === s.id && suspicious && (
                        <tr key={`${s.id}-exp`} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,0,0,0.2)' }}>
                          <td colSpan={7} style={{ padding: '1rem 1.5rem' }}>
                            <div style={{ fontSize: '0.7rem', fontWeight: 600, color: '#4A5568', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>
                              Sudo Commands Executed During Session
                            </div>
