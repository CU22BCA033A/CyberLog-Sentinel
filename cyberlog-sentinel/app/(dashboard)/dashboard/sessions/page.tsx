...
interface Session {
  id: string; username: string | null; source_ip: string | null;
  login_time: string | null; logout_time: string | null;
  duration_seconds: number | null; sudo_commands: string[]; status: string;
}
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    getLatestJobId().then(async id => {
      setJobId(id);
      if (!id) { setLoading(false); return; }
      const supabase = getSupabaseClient();
      const { data } = await supabase.from('ssh_sessions').select('*').eq('job_id', id).order('login_time', { ascending: false });
  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#8892A4', fontSize: '0.875rem', marginTop: '0.25rem' }}>{sessions.length} reconstructed sessions</p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8125rem' }}>
            ...
            <tbody>
              {loading ? Array.from({ length: 6 }).map((_, i) => (
                <tr key={i}>{Array.from({ length: 7 }).map((_, j) => <td key={j} style={{ padding: '0.75rem 1rem' }}><div className="skeleton" style={{ height: 16 }} /></td>)}</tr>
              )) : sessions.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '3rem', color: '#8892A4' }}>{jobId ? 'No sessions found in this log file.' : 'Upload a log file to see SSH sessions.'}</td></tr>
              ) : sessions.map(s => (
                <tr key={s.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: s.status === 'active' ? 'rgba(255,184,0,0.04)' : 'transparent' }}>
                  ...
                  <td style={{ padding: '0.75rem 1rem', color: '#8892A4', whiteSpace: 'nowrap' }}>{s.logout_time ? format(new Date(s.logout_time), 'HH:mm:ss') : '—'}</td>
                  <td style={{ padding: '0.75rem 1rem', color: '#E8EDF5' }}>{formatDuration(s.duration_seconds)}</td>
                  <td style={{ padding: '0.75rem 1rem' }}>
                    {s.sudo_commands?.length > 0
                      ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.75rem', color: '#FFB800' }}><AlertTriangle size={12} /> {s.sudo_commands.length} command{s.sudo_commands.length !== 1 ? 's' : ''}</span>
