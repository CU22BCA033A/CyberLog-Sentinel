...
export default function SearchPage() {
  ...
  async function runSearch(e: React.FormEvent) {
    ...
    const [ipRes, userRes, rawRes, incRes] = await Promise.all([
      supabase.from('log_events').select('source_ip').eq('job_id', jid).not('source_ip', 'is', null).ilike('source_ip', `%${q}%`).limit(500),
      supabase.from('log_events').select('username').eq('job_id', jid).not('username', 'is', null).ilike('username', `%${q}%`).limit(500),
      supabase.from('log_events').select('id,raw_line,timestamp,severity').eq('job_id', jid).ilike('raw_line', `%${q}%`).order('timestamp', { ascending: false }).limit(20),
      supabase.from('incidents').select('id,title,incident_ref,severity').eq('job_id', jid).or(`title.ilike.%${q}%,mitre_technique_id.ilike.%${q}%,description.ilike.%${q}%`).limit(10),
    ]);
    ...
    setResults({
      ips: Array.from(ipCounts.entries()).sort((a, b) => b[1] - a[1]).map(([ip, count]) => ({ ip, count })),
      users: Array.from(userCounts.entries()).sort((a, b) => b[1] - a[1]).map(([username, count]) => ({ username, count })),
  ...
  const totalResults = results ? results.ips.length + results.users.length + results.rawLines.length + results.incidents.length : 0;

  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#E8EDF5', margin: 0 }}>Global Search</h1>
        <p style={{ color: '#8892A4', fontSize: '0.875rem', marginTop: '0.25rem' }}>Search across IPs, usernames, raw log lines, incidents, and MITRE technique IDs</p>
      </div>
      ...
      <form onSubmit={runSearch} style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input value={query} onChange={e => setQuery(e.target.value)}
            placeholder="Search IP, username, log line, MITRE ID (e.g. T1110)..." autoFocus
      ...
      {!searched && !loading && (
        <div className="glass-card" style={{ padding: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {['root', 'admin', 'Failed password', 'UNION SELECT', 'script', 'passwd', 'T1110', 'T1078', 'T1190', 'locked'].map(s => (
              <button key={s} onClick={() => setQuery(s)} className="mono"
      ...
      {searched && !loading && results && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {results.incidents.length > 0 && (
            <div className="glass-card" style={{ padding: '1.25rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', fontWeight: 700, color: '#E8EDF5', margin: '0 0 0.75rem' }}>
                <Shield size={16} color="#FFB800" /> Incidents <span style={{ fontSize: '0.75rem', color: '#4A5568', fontWeight: 400 }}>({results.incidents.length})</span>
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {results.incidents.map(r => (
                  <Link key={r.id} href={`/dashboard/threats/${r.id}`}
                    style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem', borderRadius: '8px', textDecoration: 'none', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          {results.ips.length > 0 && (
            <div className="glass-card" style={{ padding: '1.25rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', fontWeight: 700, color: '#E8EDF5', margin: '0 0 0.75rem' }}>
                <Globe size={16} color="#00FF88" /> IP Addresses <span style={{ fontSize: '0.75rem', color: '#4A5568', fontWeight: 400 }}>({results.ips.length})</span>
              </h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {results.ips.map(r => (
          ...
          {results.users.length > 0 && (
            <div className="glass-card" style={{ padding: '1.25rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', fontWeight: 700, color: '#E8EDF5', margin: '0 0 0.75rem' }}>
                <User size={16} color="#00D4FF" /> Usernames <span style={{ fontSize: '0.75rem', color: '#4A5568', fontWeight: 400 }}>({results.users.length})</span>
              </h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {results.users.map(r => (
          ...
          {results.rawLines.length > 0 && (
            <div className="glass-card" style={{ padding: '1.25rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', fontWeight: 700, color: '#E8EDF5', margin: '0 0 0.75rem' }}>
                <FileText size={16} color="#8892A4" /> Raw Log Lines <span style={{ fontSize: '0.75rem', color: '#4A5568', fontWeight: 400 }}>({results.rawLines.length} shown)</span>
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {results.rawLines.map(r => (
                  <div key={r.id} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', padding: '0.625rem', background: 'rgba(0,0,0,0.3)', borderRadius: '6px', borderLeft: `3px solid ${SEVERITY_COLORS[r.severity] ?? '#4A5568'}` }}>
                    <span className="mono" style={{ fontSize: '0.7rem', color: '#4A5568', whiteSpace: 'nowrap', flexShrink: 0, paddingTop: 1 }}>
          ...
          {totalResults === 0 && (
            <div className="glass-card" style={{ padding: '3rem', textAlign: 'center' }}>
              ...
              <p style={{ color: '#8892A4', fontSize: '0.875rem', margin: 0 }}>No results for <span className="mono" style={{ color: '#E8EDF5' }}>"{query}"</span></p>
              <p style={{ color: '#4A5568', fontSize: '0.8125rem', marginTop: '0.5rem' }}>Try an IP address, username, keyword from the log, or a MITRE technique ID like T1110</p>
            </div>
          )}
        </div>
      )}
