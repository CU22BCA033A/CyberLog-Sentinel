...
import {
  Activity, AlertTriangle, Flame, Globe,
  Shield, Users, Upload, TrendingUp
} from 'lucide-react';
import {
...
function MetricCard({ label, value, icon: Icon, color, change }: {
  label: string; value: number | string; icon: React.ElementType; color: string; change?: string;
}) {
  return (
    <div className="glass-card" style={{ padding: '1.25rem', borderLeft: `3px solid ${color}` }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
        ...
        {change && <span style={{ fontSize: '0.75rem', color: '#00FF88', background: 'rgba(0,255,136,0.1)', padding: '2px 8px', borderRadius: '4px' }}>{change}</span>}
      </div>
      <div style={{ fontSize: '1.75rem', fontWeight: 800, color: '#E8EDF5', fontFamily: 'JetBrains Mono, monospace', lineHeight: 1 }}>{typeof value === 'number' ? value.toLocaleString() : value}</div>
...
export default function DashboardPage() {
  ...
  const [timelineData, setTimelineData] = useState<Array<{ hour: string; success: number; failure: number; total: number }>>([]);
  ...
  async function loadDashboard() {
    ...
    if (!jobId) {
      setStats({ totalEvents: 0, threatsDetected: 0, criticalAlerts: 0, uniqueIPs: 0, bruteForceIncidents: 0, compromisedAccounts: 0, recentJobs: jobs ?? [] });
      setLoading(false);
      return;
    }
    ...
    setSeverityData(Object.entries(sevCounts).filter(([, v]) => v > 0).map(([k, v]) => ({ name: k, value: v, color: SEVERITY_COLORS[k] })));
    ...
    setTimelineData(Object.entries(hourMap).sort().map(([h, v]) => ({ hour: h.slice(11, 16), success: v.success, failure: v.failure, total: v.success + v.failure })));
    ...
    const sortedIPs = Object.entries(ipMap).sort((a, b) => b[1].failures - a[1].failures).slice(0, 10);
    setTopIPs(sortedIPs.map(([ip, v]) => ({ ip, ...v })));
    ...
    const bruteForce = incidents.filter(i => i.status !== 'false_positive').length;
    ...
    setStats({
      ...
      criticalAlerts,
      uniqueIPs,
      bruteForceIncidents: bruteForce,
    ...
    <div>
      ...
      {!loading && !selectedJobId && (
        <div className="glass-card" style={{ padding: '3rem', textAlign: 'center' }}>
          ...
          <p style={{ color: '#8892A4', fontSize: '0.875rem', marginBottom: '1.5rem' }}>Upload a Linux auth.log file to start detecting threats and generating insights.</p>
      ...
      {(loading || selectedJobId) && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            {/* Timeline */}
            <div className="glass-card" style={{ padding: '1.25rem' }}>
              <h3 style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#E8EDF5', margin: '0 0 1rem' }}>Authentication Activity Timeline</h3>
              ...
              {loading ? <div className="skeleton" style={{ height: 200 }} /> : (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    ...
                    <Tooltip contentStyle={{ background: '#0F1520', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 12 }} />
                    <Legend formatter={(value) => <span style={{ color: '#8892A4', fontSize: 12, textTransform: 'capitalize' }}>{value}</span>} />
                  </PieChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            {loading ? <div className="skeleton" style={{ height: 200 }} /> : (
              <ResponsiveContainer width="100%" height={Math.max(200, topIPs.length * 40)}>
                <BarChart data={topIPs} layout="vertical">
                  ...
                  <YAxis type="category" dataKey="ip" width={130} tick={{ fontSize: 11, fill: '#8892A4', fontFamily: 'JetBrains Mono, monospace' }} />
                  <Tooltip contentStyle={{ background: '#0F1520', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 12 }} />
                  ...
                  {stats.recentJobs.map(job => (
                    <tr key={job.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <td style={{ padding: '0.625rem 0.5rem' }}>
                        <span className={`badge badge-${job.status === 'complete' ? 'info' : job.status === 'failed' ? 'critical' : 'medium'}`}>{job.status}</span>
                      </td>
